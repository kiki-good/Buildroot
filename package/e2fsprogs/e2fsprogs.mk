...
$(E2FSPROGS_DIR)/.configured: $(E2FSPROGS_DIR)/.unpacked
	(cd $(E2FSPROGS_DIR); rm -rf config.cache; \
		...
		--with-linker="$(TARGET_CROSS)ld $(TARGET_SYSROOT)" \
		--prefix=/usr \
		--exec-prefix=/usr \
		--bindir=/bin \
		--sbindir=/sbin \
		...
		--enable-dynamic-e2fsck --disable-swapfs \
...
$(E2FSPROGS_DIR)/$(E2FSPROGS_BINARY): $(E2FSPROGS_DIR)/.configured
	$(MAKE1) -C $(E2FSPROGS_DIR) ALL_LDFLAGS=-pthread
...
ifneq ($(BR2_SKY_BUILD_TYPE), "debug")
	rm -rf $(TARGET_DIR)/sbin/uuidd
	rm -rf $(TARGET_DIR)/sbin/mklost+found
	rm -rf $(TARGET_DIR)/sbin/mkfs.ext4dev
endif
	touch -c $@
