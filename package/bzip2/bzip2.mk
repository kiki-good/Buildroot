...
ifeq ($(BR2_PACKAGE_DIAGNOSTICS),y)
	mkdir -p $(STAGING_DIR)/usr/lib && cp -af $(BZIP2_DIR)/libbz2.so.$(BZIP2_VERSION) $(STAGING_DIR)/usr/lib/
else
...
ifeq ($(BR2_PACKAGE_DIAGNOSTICS),y)
	mkdir -p $(TARGET_DIR)/usr/lib && cp -af $(BZIP2_DIR)/libbz2.so.$(BZIP2_VERSION) $(TARGET_DIR)/usr/lib/
else
