 ...
 *
 * Modified: David Roberts - Sky Brentwood.  9th May 2011
 * Removed sync command causes problems in bigbuild systems with large cache to sync.
 *
 */
...
int process_table(FILE *table, char *rootdir)
{
	char *line = NULL;
	int linenum = 0;
	int ret = 0;
	...
	pwd = getenv("PWD");
	if (pwd) {
		strncpy(include, pwd, 4096);
		incidx = strlen(pwd);
		include[incidx] = '/';
		incidx++;
		sprintf(incfmt, " include %%%ds", 4096 - incidx);
	}
	...
	while ((line = bb_get_chomped_line_from_file(table))) {
		...
		if (sscanf(line, incfmt, &include[incidx]) > 0)
		{
			FILE *subtable;
			char *incptr = 0;
			...
			if (include[incidx] != '/') {
				...
				incptr = &include[incidx];
			}
				
			if ((subtable = fopen(incptr, "r")) == NULL) {
				printf("Unable to open include %s\n", incptr);	
				continue;
			}

			printf("Including additional device table %s\n", incptr);
			ret = process_table(subtable, rootdir);

			continue;
		}

		if ((2 > sscanf(line, "%100s %c %o %40s %40s %u %u %u %u %u", name,
		...
		} else
		{
			...
			if (type == 'p') {
				...
				bb_error_msg("line %d: Unsupported file type %c for \nline:%s\nname:%s, len = %d", linenum, type, line, name, strlen(name) );
				ret = EXIT_FAILURE;
				goto loop;
			}
