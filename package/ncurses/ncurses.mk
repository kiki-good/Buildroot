...
NCURSES_CAT:=$(ZCAT)
NCURSES_BINARY_SOURCE:=ncurses-$(NCURSES_VERSION)-binary.tar.bz2
NCURSES_BOARD_PATH:=$(strip  $(subst ",, $(BR2_BOARD_PATH)"))
NCURSES_BINARY_DIR:=$(NCURSES_BOARD_PATH)/binaries
NCURSES_BINARY_TEMP_DIR:=$(NCURSES_BINARY_DIR)/ncurses_temp
NCURSES_INSTALL_BINARY:=$(NCURSES_BINARY_DIR)/$(NCURSES_BINARY_SOURCE)
...
$(STAGING_DIR)/lib/libncurses.so.$(NCURSES_VERSION): $(NCURSES_DIR)/lib/libncurses.so.$(NCURSES_VERSION)
	...
	mkdir -p $(PWD)/$(NCURSES_BINARY_TEMP_DIR)/bin \
		 $(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/lib
	$(MAKE1) \
	    prefix=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/ \
	    exec_prefix=$(PWD)/$(NCURSES_BINARY_TEMP_DIR) \
	    bindir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/bin \
	    sbindir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/sbin \
	    libexecdir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/lib \
	    datadir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/share \
	    sysconfdir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/etc \
	    localstatedir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/var \
	    libdir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/lib \
	    infodir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/info \
	    mandir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/man \
	    includedir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/include \
	    gxx_include_dir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/include/c++ \
	    ticdir=$(PWD)/$(NCURSES_BINARY_TEMP_DIR)/usr/share/terminfo \
	    -C $(NCURSES_DIR) install
	chmod a-x $(NCURSES_DIR)/lib/libncurses.so*
	tar cjvf $(NCURSES_BINARY_DIR)/$(NCURSES_BINARY_SOURCE) -C $(NCURSES_BINARY_TEMP_DIR) .
	rm -rf $(NCURSES_BINARY_TEMP_DIR)
	touch -c $@
...
ncurses-binary: 
	tar -xjvf $(NCURSES_BINARY_DIR)/$(NCURSES_BINARY_SOURCE) -C $(STAGING_DIR)/
	mkdir -p $(TARGET_DIR)/lib $(TARGET_DIR)/usr/lib 
	cp -dpf $(STAGING_DIR)/lib/libncurses.so* $(TARGET_DIR)/lib/
ifeq ($(BR2_PACKAGE_NCURSES_TARGET_PANEL),y)
	cp -dpf $(STAGING_DIR)/lib/libpanel.so* $(TARGET_DIR)/usr/lib/
endif
ifeq ($(BR2_PACKAGE_NCURSES_TARGET_FORM),y)
	cp -dpf $(STAGING_DIR)/lib/libform.so* $(TARGET_DIR)/usr/lib/
endif
ifeq ($(BR2_PACKAGE_NCURSES_TARGET_MENU),y)
	cp -dpf $(STAGING_DIR)/lib/libmenu.so* $(TARGET_DIR)/usr/lib/
endif
	ln -snf /usr/share/terminfo $(TARGET_DIR)/usr/lib/terminfo
	mkdir -p $(TARGET_DIR)/usr/share/terminfo/x
	cp -dpf $(STAGING_DIR)/usr/share/terminfo/x/xterm $(TARGET_DIR)/usr/share/terminfo/x
...
ncurses-headers-binary: 
	mkdir -p $(TARGET_DIR)/usr/include
	cp -dpf $(STAGING_DIR)/usr/include/curses.h $(TARGET_DIR)/usr/include/ncurses.h
	cp -dpf $(STAGING_DIR)/usr/include/ncurses_dll.h $(TARGET_DIR)/usr/include/ncurses_dll.h
	cp -dpf $(STAGING_DIR)/usr/include/term.h $(TARGET_DIR)/usr/include/
	cp -dpf $(STAGING_DIR)/usr/include/unctrl.h $(TARGET_DIR)/usr/include/
	cp -dpf $(STAGING_DIR)/usr/include/termcap.h $(TARGET_DIR)/usr/include/
	cp -dpf $(STAGING_DIR)/lib/libncurses.a $(TARGET_DIR)/usr/lib/
	(cd $(TARGET_DIR)/usr/lib; \
	 ln -fs libncurses.a libcurses.a; \
	 ln -fs libncurses.a libtermcap.a; \
	)
...
ncurses-binary-clean:
	rm $(NCURSES_INSTALL_BINARY)
