...
GST1_PLUGINS_BAD_SITE = http://localhost/
...
GST1_PLUGINS_BAD_AUTORECONF_ENV = AUTOPOINT=true
GST1_PLUGINS_BAD_AUTORECONF_OPT = -I $(@D)/common/m4

GST1_PLUGINS_BAD_DIR=$(BUILD_DIR)/gst1-plugins-bad-$(GST1_PLUGINS_BAD_VERSION)
GST1_PLUGINS_BAD_BINARY_SOURCE:=gst1-plugins-bad-$(GST1_PLUGINS_BAD_VERSION)-binary.tar.bz2
GST1_PLUGINS_BAD_BOARD_PATH:=$(strip  $(subst ",, $(BR2_BOARD_PATH)"))
GST1_PLUGINS_BAD_BINARY_DIR:=$(GST1_PLUGINS_BAD_BOARD_PATH)/binaries
GST1_PLUGINS_BAD_INSTALL_BINARY:=$(GST1_PLUGINS_BAD_BINARY_DIR)/$(GST1_PLUGINS_BAD_BINARY_SOURCE)
GST1_PLUGINS_BAD_TEMP_DIR:=$(GST1_PLUGINS_BAD_BINARY_DIR)/gst1-plugins-bad_tmp

define GST1_PLUGINS_BAD_PREBUILT_BINARIES
	mkdir -p $(GST1_PLUGINS_BAD_TEMP_DIR)
	$(MAKE) DESTDIR=$(PWD)/$(GST1_PLUGINS_BAD_TEMP_DIR) -C $(GST1_PLUGINS_BAD_DIR) install
	tar cjvf $(GST1_PLUGINS_BAD_BINARY_DIR)/$(GST1_PLUGINS_BAD_BINARY_SOURCE) -C $(GST1_PLUGINS_BAD_TEMP_DIR) .
	rm -rf $(GST1_PLUGINS_BAD_TEMP_DIR)
endef


GST1_PLUGINS_BAD_POST_INSTALL_STAGING_HOOKS += GST1_PLUGINS_BAD_PREBUILT_BINARIES
...
gst1-plugins-bad-binary:
	tar -xjvf $(GST1_PLUGINS_BAD_BINARY_DIR)/$(GST1_PLUGINS_BAD_BINARY_SOURCE) -C $(STAGING_DIR)/
	tar -xjvf $(GST1_PLUGINS_BAD_BINARY_DIR)/$(GST1_PLUGINS_BAD_BINARY_SOURCE) -C $(TARGET_DIR)/

gst1-plugins-bad-binary-clean:
	rm $(GST1_PLUGINS_BAD_INSTALL_BINARY)

ifeq ($(wildcard $(GST1_PLUGINS_BAD_INSTALL_BINARY)),)
gst1-plugins-bad: $(eval $(autotools-package)) $(eval $(call AUTOTARGETS,package/multimedia/gstreamer1,gst1-plugins-bad))
else
gst1-plugins-bad: gst1-plugins-bad-binary
endif

ifeq ($(BR2_PACKAGE_GST1_PLUGINS_BAD),y)
TARGETS+=gst1-plugins-bad
endif
