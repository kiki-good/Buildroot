...
GST1_PLUGINS_GOOD_SITE = http://localhost/
GST1_PLUGINS_GOOD_INSTALL_STAGING = YES
GST1_PLUGINS_GOOD_LICENSE_FILES = COPYING
GST1_PLUGINS_GOOD_LICENSE = LGPLv2.1+
GST1_PLUGINS_GOOD_AUTORECONF = YES
GST1_PLUGINS_GOOD_AUTORECONF_ENV = AUTOPOINT=true

GST1_PLUGINS_GOOD_DIR=$(BUILD_DIR)/gst1-plugins-good-$(GST1_PLUGINS_GOOD_VERSION)
GST1_PLUGINS_GOOD_BINARY_SOURCE:=gst1-plugins-good-$(GST1_PLUGINS_GOOD_VERSION)-binary.tar.bz2
GST1_PLUGINS_GOOD_BOARD_PATH:=$(strip  $(subst ",, $(BR2_BOARD_PATH)"))
GST1_PLUGINS_GOOD_BINARY_DIR:=$(GST1_PLUGINS_GOOD_BOARD_PATH)/binaries
GST1_PLUGINS_GOOD_INSTALL_BINARY:=$(GST1_PLUGINS_GOOD_BINARY_DIR)/$(GST1_PLUGINS_GOOD_BINARY_SOURCE)
GST1_PLUGINS_GOOD_TEMP_DIR:=$(GST1_PLUGINS_GOOD_BINARY_DIR)/gst1-plugins-good_tmp

define GST1_PLUGINS_GOOD_PREBUILT_BINARIES
	mkdir -p $(GST1_PLUGINS_GOOD_TEMP_DIR)
	$(MAKE) DESTDIR=$(PWD)/$(GST1_PLUGINS_GOOD_TEMP_DIR) -C $(GST1_PLUGINS_GOOD_DIR) install
	tar cjvf $(GST1_PLUGINS_GOOD_BINARY_DIR)/$(GST1_PLUGINS_GOOD_BINARY_SOURCE) -C $(GST1_PLUGINS_GOOD_TEMP_DIR) .
	rm -rf $(GST1_PLUGINS_GOOD_TEMP_DIR)
endef

GST1_PLUGINS_GOOD_POST_INSTALL_STAGING_HOOKS += GST1_PLUGINS_GOOD_PREBUILT_BINARIES

GST1_PLUGINS_GOOD_CONF_ENV = \
...
gst1-plugins-good-binary:
	tar -xjvf $(GST1_PLUGINS_GOOD_BINARY_DIR)/$(GST1_PLUGINS_GOOD_BINARY_SOURCE) -C $(STAGING_DIR)/
	tar -xjvf $(GST1_PLUGINS_GOOD_BINARY_DIR)/$(GST1_PLUGINS_GOOD_BINARY_SOURCE) -C $(TARGET_DIR)/

gst1-plugins-good-binary-clean:
	rm $(GST1_PLUGINS_GOOD_INSTALL_BINARY)

ifeq ($(wildcard $(GST1_PLUGINS_GOOD_INSTALL_BINARY)),)
gst1-plugins-good: $(eval $(autotools-package)) $(eval $(call AUTOTARGETS,package/multimedia/gstreamer1,gst1-plugins-good))
else
gst1-plugins-good: gst1-plugins-good-binary
endif

ifeq ($(BR2_PACKAGE_GST1_PLUGINS_GOOD),y)
TARGETS+=gst1-plugins-good
endif
