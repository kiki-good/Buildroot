...
xdr_remote_domain_get_info_ret (XDR *xdrs, remote_domain_get_info_ret *objp)
{
         ...
         if (!xdr_uint64_t (xdrs, &objp->cpuTime))
                 return FALSE;
         if (!xdr_uint64_t (xdrs, &objp->maxGpuMem))
                 return FALSE;
         if (!xdr_uint64_t (xdrs, &objp->gpuMemory))
                 return FALSE;
        return TRUE;
}
...
bool_t
xdr_remote_domain_mount_fuse_fs_args (XDR *xdrs, remote_domain_mount_fuse_fs_args *objp)
{
        char **objp_cpp0 = (char **) (void *) &objp->mountoptions.mountoptions_val;

         if (!xdr_remote_nonnull_domain (xdrs, &objp->dom))
                 return FALSE;
         if (!xdr_remote_nonnull_string (xdrs, &objp->mountpoint))
                 return FALSE;
         if (!xdr_array (xdrs, objp_cpp0, (u_int *) &objp->mountoptions.mountoptions_len, REMOTE_DOMAIN_FUSE_MOUNTOPTIONS_MAX,
                sizeof (remote_nonnull_string), (xdrproc_t) xdr_remote_nonnull_string))
                 return FALSE;
        return TRUE;
}
